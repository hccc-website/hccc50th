...
// Includes Javascripts for Frontpage Slider (pluggable)
add_action('wp_head', 'themezee_jscript_frontpage_slider');

if ( ! function_exists( 'themezee_jscript_frontpage_slider' ) ) :
function themezee_jscript_frontpage_slider() {
	...
	// Check if Frontpage Slider is activated
	$options = get_option('themezee_options');
	if(isset($options['themeZee_frontpage_slider_active']) and $options['themeZee_frontpage_slider_active'] == 'true') :
	
		// Select Frontpage Slider Mode
		switch($options['themeZee_frontpage_slider_mode']) {
			case 0:
				// Slider Effect: CurtainX
				$jscript = "<script type=\"text/javascript\">
				//<![CDATA[
					jQuery(document).ready(function($) {
						$('#frontpage_slideshow')
			...
			break;
			default:
				// Default Slider Effect: curtainX
				$jscript =  "<script type=\"text/javascript\">
				//<![CDATA[
					jQuery(document).ready(function($) {
						$('#frontpage_slideshow')
